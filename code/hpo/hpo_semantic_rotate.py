...
if __name__ == "__main__":
    ...
    model_kwargs_range = dict(
        type_dim=dict(type=int, scale="power", base=2, low=8, high=10),
        ent_dim=dict(type=int, scale="power_two", low=9, high=10),
        usepretrained=dict(
            type="categorical",
            choices=["bert-base-uncased", "bert-large-uncased", None],
        ),
        freeze_matrix=dict(type=bool),
    )
    loss_kwargs_ranges = dict(
        margin=dict(type=int, low=6, high=12, q=3),
    ...
    negative_sampler_kwargs_ranges = dict(
        num_negs_per_pos=dict(type=int, scale="power_two", low=8, high=9)
    )
    ...
    pipeline_result = hpo_pipeline(
        ...
        storage="sqlite:///../../models/{}.db".format(dataset + "_semantic_rotate"),
        load_if_exists=True,
