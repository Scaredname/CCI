...
class RSETC(TypeFramework):
    ...
    def __init__(
        ...
        freeze_matrix=False,
        add_ent_type=True,
        init_preference_one=False,
        learn_ents_types=False,
        **kwargs,
    ) -> None:
        super().__init__(**kwargs)

        self.ents_types = None
        self.ents_types_weight = torch.nn.parameter.Parameter(
            torch.as_tensor(
                self.triples_factory.ents_types,
                dtype=self.data_type,
                device=self.device,
            ),
            requires_grad=learn_ents_types,
        )
        self.add_ent_type = add_ent_type
        if not init_preference_one:
            rel_types = self.triples_factory.rels_types
        else:
            rel_types = self.triples_factory.rels_types_mask
            self.ents_types_weight = torch.nn.parameter.Parameter(
                torch.as_tensor(
                    self.triples_factory.ents_types_mask,
                    dtype=self.data_type,
                    device=self.device,
                ),
                requires_grad=learn_ents_types,
        ...
        self.rel_type_t_weights = self._build_weights_representations(
            ...
            representations=None,
            representations_kwargs=dict(
                initializer=PretrainedInitializer(tensor=rel_types[1]),
                constrainer="clamp",
                constrainer_kwargs=dict(min=0),
                trainable=not freeze_matrix,
                shape=type_num,
            ...
            if self.activation_weight:
                head_type_emb_tensor = torch.matmul(
                    self.activation_function(
                        self.type_weight_temperature
                        * (ents_types[h] + rels_types_h[r_h])
                    ),
                    type_emb,
                )
                tail_type_emb_tensor = torch.matmul(
                    self.activation_function(
                        self.type_weight_temperature
                        * (ents_types[t] + rels_types_t[r_t])
                    ),
                    type_emb,
                )
            else:
                head_type_emb_tensor = torch.matmul(
                    ents_types[h] + rels_types_h[r_h], type_emb
                )
                tail_type_emb_tensor = torch.matmul(
                    ents_types[t] + rels_types_t[r_t], type_emb
                )
        else:
            if self.activation_weight:
                head_type_emb_tensor = torch.matmul(
                    self.activation_function(
                        self.type_weight_temperature * rels_types_h[r_h]
                    ),
                    type_emb,
                )
                tail_type_emb_tensor = torch.matmul(
                    self.activation_function(
                        self.type_weight_temperature * rels_types_t[r_t]
                    ),
                    type_emb,
                )
            else:
