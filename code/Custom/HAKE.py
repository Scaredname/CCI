...
def hake_interaction(
    ...
    if estimate_cost_of_sequence(h.shape, r.shape) < estimate_cost_of_sequence(
        r.shape, t.shape
    ):
    ...
    r_score = torch.norm(r_score, p=2, dim=2) * modulus_weight
    return -(phase_score + r_score).sum(dim=-1)


class HAKEInteraction(
    FunctionalInteraction[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]
):
