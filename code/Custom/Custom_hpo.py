...
"""Hyper-parameter optimiziation in PyKEEN. judge conditions for _negative_sampler_kwargs changed"""
...
from pykeen.lr_schedulers import (
    LRScheduler,
...
from pykeen.training import LCWATrainingLoop, TrainingLoop, training_loop_resolver
...
class Objective:
    ...
    def _update_stopper_callbacks(
        ...
        def _result_callback(
            _early_stopper: EarlyStopper, result: Union[float, int], epoch: int
        ) -> None:
        ...
        def _stopped_callback(
            _early_stopper: EarlyStopper, _result: Union[float, int], epoch: int
        ) -> None:
    ...
    def __call__(self, trial: Trial) -> Optional[float]:
        ...
        try:
            ...
            logger.warning(
                "using a loss function with no hpo_default field: %s", self.loss
            )
        ...
        if self.training_loop is LCWATrainingLoop:
            ...
            parent_params = inspect.signature(
                self.negative_sampler.__bases__[0]
            ).parameters
        ...
        try:
            ...
            if self.save_model_directory:
                model_directory = os.path.join(
                    self.save_model_directory, str(trial.number)
                )
...
class HpoPipelineResult(Result):
        ...
        for field in dataclasses.fields(self.objective):
            ...
            if field.name.endswith("_kwargs"):
                ...
                if issubclass(field_value, ResultTracker):
                    ...
                    logger.error(
                        f"Invalid value for field {field.name}: {field_value!r}"
                    )
    ...
    def save_to_ftp(self, directory: str, ftp: ftplib.FTP):
        ...
        ftp.storbinary(
            f"STOR {best_config_path}", get_json_bytes_io(self._get_best_study_config())
        )
    ...
    def save_to_s3(self, directory: str, bucket: str, s3=None) -> None:
        ...
        best_config_path = os.path.join(
            directory, "best_pipeline", "pipeline_config.json"
        )
        s3.upload_fileobj(
            get_json_bytes_io(self._get_best_study_config()), bucket, best_config_path
        )
    ...
    def replicate_best_pipeline(
        ...
        *,
        ...
        if "use_testing_data" in config:
            raise ValueError(
                "use_testing_data not be set in the configuration at at all!"
            )
...
def hpo_pipeline_from_path(
    path: Union[str, pathlib.Path], **kwargs
) -> HpoPipelineResult:
...
def hpo_pipeline(
    *,
    ...
    # 3. Loss
    loss_cls: Type[Loss] = (
        model_cls.loss_default if loss is None else loss_resolver.lookup(loss)
    )
    ...
    if lr_scheduler is not None:
        ...
        study.set_user_attr(
            "lr_scheduler", lr_scheduler_resolver.normalize_cls(lr_scheduler_cls)
        )
    ...
    logger.info(
        "Filter validation triples when testing: %s", filter_validation_when_testing
    )
...
def suggest_discrete_power_int(
    trial: Trial, name: str, low: int, high: int, base: int = 2
) -> int:
    ...
    *,
    ...
    if dataset is not None:
        if training is not None or testing is not None or validation is not None:
            ...
            isinstance(dataset, type) and issubclass(dataset, Dataset)
        ):
