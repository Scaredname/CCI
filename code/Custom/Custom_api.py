...
from pykeen.evaluation.ranking_metric_lookup import normalize_flattened_metric_results
...
def triple_hash(*triples: MappedTriples) -> Mapping[str, str]:
    ...
    return dict(
        sha512=hashlib.sha512(
            str(sorted(sum((t.tolist() for t in triples), []))).encode("utf8")
        ).hexdigest()
    )
...
class PipelineResult(Result):
    ...
    def save_to_directory(
        ...
        **_kwargs,
        ...
        if save_training:
            self.training.to_path_binary(
                directory.joinpath(self.TRAINING_TRIPLES_FILE_NAME)
            )
...
def replicate_pipeline_from_config(
    ...
    **kwargs,
    ...
    pipeline_results = (
        pipeline_from_config(config, discard_seed=not keep_seed, **kwargs)
...
def compare_results(df: pd.DataFrame, significance_level: float = 0.01) -> pd.DataFrame:
    ...
    p_values = [
        test(df.loc[~original_mask, metric], original[metric], **kwargs).pvalue
...
def save_pipeline_results_to_directory(
    *,
    ...
    losses_df.to_csv(
        directory.joinpath("all_replicates_losses.tsv"), sep="\t", index=False
    )

    if result_comparator.is_non_empty():
        metric_df = result_comparator.get_df()
        metric_df.to_csv(
            directory.joinpath("all_replicates_metrics.tsv"), sep="\t", index=False
        )
...
def pipeline_from_config(
    ...
    **kwargs,
    ...
    if discard_seed and "random_seed" in pipeline_kwargs:
        ...
        logger.info(
            f"Ignoring random_seed={random_seed}. You need to explicitly disable this, if unintended."
        )
...
def _build_model_helper(
    *,
    ...
    if regularizer is not None:
        ...
        model_kwargs["regularizer"] = regularizer_resolver.make(
...
def _handle_training(
    *,
    ...
    if "evaluation_triples_factory" not in stopper_kwargs:
    ...
    _result_tracker.log_metrics(
        metrics=dict(total_training=training_end_time), step=step, prefix="times"
    )
...
def _handle_evaluation(
    *,
    ...
    if evaluator_instance.filtered:
        ...
        additional_filter_triples_names["training"] = triple_hash(
            training.mapped_triples
        )
        ...
        if popped_additional_filter_triples:
            additional_filter_triples_names["custom"] = triple_hash(
                *popped_additional_filter_triples
            )
        ...
        if (
            ...
            additional_filter_triples_names["validation"] = triple_hash(
        ...
        params=dict(
            evaluation_kwargs={
                k: (
                    ...
                    if k == "additional_filter_triples"
                    else v
                )
    ...
    step = training_kwargs.get("num_epochs")
    _result_tracker.log_metrics(
        metrics=dict(final_evaluation=evaluate_end_time), step=step, prefix="times"
    )
    _result_tracker.log_metrics(
    ...
    while True:
        try:
            ...
            # restarted with PyKEEN's automatic memory optimization
                ...
                logging.warning(
                    "Will activate the built-in PyKEEN memory optimization to find a suitable batch size."
                )
